...
当前状态：`IN_PROGRESS`（2026-09-23 已保存迁移演练 WIP 检查点）。

已完成并推送的独立阶段：候选镜像构建、后端/前端全量测试、Compose 空卷启动、真实依赖 readiness、最小权限、内部端口隔离、依赖故障恢复，以及 MySQL/Redis/上传卷持久化。旧库迁移脚本已通过合成历史结构的首次迁移、幂等重跑、结构负测和清理验证，但最终复审还有以下未完成项：

1. 失败路径也必须执行分支、HEAD、工作区状态和源码摘要终态复核；
2. 自定义证据路径必须拒绝通过 symlink/junction/ReparsePoint 绕回仓库；
3. 数据摘要必须覆盖 legacy fixture 的全部七张表，包括三个初始空表；
4. 完成上述修复后重新生成干净工作区迁移证据；
5. 完成 MySQL、Redis、上传文件同一停止写入点的联合恢复，并用上一版镜像验证回滚。

在这些事项完成前，DEPLOY-04 不得标记为 `DONE`，也不得进入服务器部署。
