...
这套文档用于把“校园资料共享与检索平台”的开发记录转换为一条能在 Java 后端面试中讲清楚、经得住追问的项目叙事。
...
本项目当前的检索能力基于 MySQL 动态 SQL，实现多条件检索、排序和热门搜索词统计，尚未接入 Elasticsearch、向量检索、大模型或语义搜索。面试中应按实际实现说明搜索能力及其边界。
