...
7. 发现 `docs/api/api-reference.md`、前端文档和真实后端代码严重不一致；
...
## 队列完成后的浏览器验收修复（2026-07-13）

- 队列 T01-T47 保持全部完成。本次不新增队列任务，仅修复浏览器验收发现的前端 API 版本前缀配置问题。
- 根因：`VITE_API_BASE_URL` 为 `/api`，与后端实际 `/api/v1` 不一致；已同步请求层缺省值、开发示例配置和运行手册。
- 验收：请求层全量单测与生产构建通过；真实 Edge 已完成游客、普通用户和管理员演示流程。
