...
| F19 | T19 | 已完成 | 收藏 API 专项测试 3/3 通过；`npm run build` 通过 | F20 / T20：详情页收藏操作 |
| F20 | T20 | 已完成 | 详情页收藏组件测试 2/2 通过；`npm run build` 通过 | F21 / T21：下载 API 和文件流工具 |
| F21 | T21 | 已完成 | 下载 API 与文件工具测试 4/4 通过；`npm run build` 通过 | F22 / T22：详情页两步下载 |
| F22 | T22 | 已完成 | 资料详情页下载组件测试 3/3 通过；`npm run build` 通过 | F23 / T23：MD5 工具和文件 API |
| F23 | T23 | 已完成 | MD5 与文件 API 专项测试 3/3 通过；`npm run build` 通过 | F24 / T24：上传页文件步骤 |
| F24 | T24 | 已完成 | 上传页组件测试 1/1 通过；`npm run build` 通过 | F25 / T25：资料元数据表单 |
| F25 | T25 | 已完成 | 上传页组件测试 2/2 通过；`npm run build` 通过；分类列表由公开分类接口加载 | F26 / T26：创建资料和结果跳转 |
...
| F30 | T30 | 已完成 | 我的收藏页面测试 1/1 通过；`npm run build` 通过；取消收藏后刷新列表由 Mock 验证 | F31 / T31：我的下载 API |
| F31 | T31 | 已完成 | 用户 API 测试 5/5 通过；`npm run build` 通过；Token 与分页 query 经 Mock 验证 | F32 / T32：我的下载页面 |
| F32 | T32 | 已完成 | 我的下载页面测试 1/1、全量测试 45/45 通过；`npm run build` 通过；再次下载两步流程经 Mock 验证 | F33 / T33：管理员审核只读 API（本轮阶段上限暂停） |
| F33 | T33 | 已完成 | 管理员资料 API 测试 2/2 通过；`npm run build` 通过；Token、query 和审核流水路径经 Mock 验证 | F34 / T34：待审核列表页面 |
| F34 | T34 | 已完成 | 待审核页面测试 1/1 通过；`npm run build` 通过；只读列表和审核流水请求经 Mock 验证 | F35 / T35：审核通过和拒绝 API |
| F35 | T35 | 已完成 | 管理员资料 API 测试 4/4 通过；`npm run build` 通过；通过/拒绝 path、body 和 Token 经 Mock 验证 | F36 / T36：审核页面写操作 |
| F36 | T36 | 已完成 | 待审核页面测试 3/3 通过；`npm run build` 通过；二次确认、拒绝原因校验和 409 错误展示经 Mock 验证 | F37 / T37：发布资料只读管理页 |
| F37 | T37 | 已完成 | 发布资料页面测试 1/1 通过；`npm run build` 通过；只展示公开搜索返回的已通过资料 | F38 / T38：下架 API |
| F38 | T38 | 已完成 | 管理员资料 API 测试 5/5 通过；`npm run build` 通过；下架 path、body 和 Token 经 Mock 验证 | F39 / T39：下架和审核流水页面操作 |
| F39 | T39 | 已完成 | 发布资料页面测试 2/2 通过；`npm run build` 通过；下架原因、确认、刷新和流水请求经 Mock 验证 | F40 / T40：管理员总榜重建 API |
| F40 | T40 | 已完成 | 管理员排行榜 API 测试 1/1 通过；`npm run build` 通过；无参数、Token 和 `data=null` 经 Mock 验证 | F41 / T41：排行榜运维页面 |
| F41 | T41 | 已完成 | 排行榜运维页面测试 1/1 通过；`npm run build` 通过；周期、确认重建和无虚构进度经 Mock 验证 | F42 / T42：路由守卫 |
| F42 | T42 | 已完成 | 路由守卫测试 3/3 通过；`npm run build` 通过；游客、普通用户和管理员跳转经内存路由验证 | F43 / T43：导航和通用状态收口 |
| F43 | T43 | 已完成 | Layout 与首页状态测试 6/6、全量前端测试 65/65 通过；`npm run build` 通过；固定导航和排行榜重试入口经组件测试验证 | F44 / T44：真实后端认证与上传审核验收 |
| F44 | T44 | 已完成 | 真实验证注册、登录、MD5 预检、首次上传、秒传、创建待审核资料、管理员审核、公开详情和退出黑名单；用户提供 PDF 已审核通过并落入独立上传目录 | `npm run test:unit` 65/65、`npm run build`、后端 `mvnw -DskipTests compile` 通过；F45 / T45：搜索收藏下载闭环 |
| F45 | T45 | 已完成 | 已用审核通过的 PDF 验证搜索隔离、收藏、两步下载、重复下载和 Redis 限流 | 真实接口验证、`npm run test:unit`（31 文件、65 用例）和 `npm run build` 均通过 |
| F46 | T46 | 已完成 | 已验证排行榜热度、总榜重建、管理员下架和角色拒绝 | PDF 保持公开；测试资料下架后已退出详情、搜索和热榜；全量前端测试与构建均通过 |
| F47 | T47 | 已完成 | 已整理前端启动说明和最终验证记录 | `npm ci` 审计 0 漏洞；全量 65 用例、生产构建和 Vite 首页 HTTP 200 均通过 |
