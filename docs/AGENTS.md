...
每次修改完成后，必须按第 20 节《每次任务完成标准》输出完成总结，至少覆盖：修改了哪些文件与类、涉及哪些接口/数据库表/Redis Key、核心逻辑是什么、如何验证或测试、是否更新文档，以及建议的 Git commit message。
...
* 验证通过后，必须立即执行一次 Git commit，并推送到当前开发分支。
...
模块内小功能开发阶段不强制编写新的测试代码；等主要模块开发完成后，再统一补充自动化测试代码。

每个模块完成后，需要至少给出测试方式，并优先补齐自动化测试覆盖。
...
模块内小功能提交前，可以先运行已有可用测试、编译命令或其他轻量验证；如果当前无法验证，必须在总结中说明原因。
...
8. 数据库变更已记录；
9. 接口文档已更新；
10. 如果本次任务完成整个模块，模块文档与模块开发流程文档已统一更新；模块内小功能阶段不强制更新模块开发流程文档；
11. 给出本次验证方式；如果本次任务完成整个模块，还需要给出测试方式；
...
模块开发流程文档是模块完成后的统一沉淀文档。

除非用户明确要求，否则模块开发过程中不再要求每完成一个小功能就更新该文档；应在整个模块开发完成后，再基于真实代码一次性生成或更新模块开发流程文档。
...
该文档不应该只记录“做了什么”，还必须提供一组用户可以直接复制给 Agent 的提示词，用于后续复现开发步骤、接手遗留任务或补充扩展功能。

### 1. 生成时机

在开始开发一个新模块前，不强制生成或更新模块开发流程文档；如已有初稿，可作为开发参考。

在模块开发过程中，每完成一个小功能后，仍需按模块小功能提交推送规则完成已有测试或编译等验证、提交和推送，但不要求同步更新模块开发流程文档，也不强制新增测试代码。

在整个模块开发完成后，必须统一生成或更新模块开发流程文档，并补全文档中的测试记录、修改文件、接口文档、面试可讲点和后续优化方向。
...
模块开发流程文档不能只记录“做了什么”，还必须提供一组用户可以直接复制给 Agent 的提示词，用于复盘真实开发顺序、指导后续接手或扩展当前模块。

要求：

1. 提示词必须集中放在文档固定小节，标题统一为“分步骤开发提示词”。
2. 提示词必须分步骤，每一步对应一个最小可执行任务，严禁用一条提示词要求一次性完成整个模块。
3. 步骤顺序必须与模块真实开发顺序一致，推荐顺序：数据库表与实体 → Mapper → DTO / VO → Service → Controller → 权限与校验 → 主要模块完成后统一补测试 → 文档更新。
...
7. 最后一条提示词必须是“模块完成后统一更新本模块开发流程文档”，用于保证模块交付时文档与代码一致。
...
- 涉及表：xxx。
- 完成后记录验证结果和提交信息；模块全部完成后再统一补测试并更新模块开发流程文档。
