...
- `frontend/` 已完成 T01-T39：发布资料页已支持审核流水、原因必填和二次确认下架；下架成功后重新读取公开搜索列表。页面测试 2/2 与构建通过；后端 8080 端口未监听，真实联调待补；当前可按队列继续 T40。
...
2. 读取队列中当前 `IN_PROGRESS` 的 T40；完成后将其标记为 DONE 并开始编号最小的 TODO（T41）。
3. 当前仅执行 T40；完成测试、更新计划/队列/状态文档、提交并推送后，才继续 T41。
