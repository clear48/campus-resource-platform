...
- `frontend/` 已完成 T01-T40：总榜重建 API 已通过 Token、无参数和 `data=null` Mock 验证，不显示虚构进度；后端 8080 端口未监听，真实联调待补；当前可按队列继续 T41。
...
2. 读取队列中当前 `IN_PROGRESS` 的 T41；完成后将其标记为 DONE 并开始编号最小的 TODO（T42）。
3. 当前仅执行 T41；完成测试、更新计划/队列/状态文档、提交并推送后，才继续 T42。
