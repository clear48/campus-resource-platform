...
已完成并推送的独立阶段：候选镜像构建、后端/前端全量测试、Compose 空卷启动、真实依赖 readiness、最小权限、内部端口隔离、依赖故障恢复，以及 MySQL/Redis/上传卷持久化。旧库迁移脚本已经覆盖合成历史结构的首次迁移、幂等重跑、七张 legacy 表全列摘要、结构负测、失败路径源码终态复核、证据路径重解析点防护和资源清理。

剩余工作：

1. 在迁移修复提交后重新生成干净工作区迁移证据；
2. 完成 MySQL、Redis、上传文件同一停止写入点的联合恢复，并用上一版镜像验证回滚。
