...
- `frontend/` 已完成 T01-T14；首页排行榜组件测试 1/1 与前述基础、认证、排行榜 API 测试均通过；后端 8080 端口未监听，真实联调待补；当前可按队列继续 T15。
...
2. 找到第一个 `TODO`：当前为 `T16`（T15 正在执行）。
3. 当前仅执行 T15；完成测试、更新计划/队列/状态文档、提交并推送后，才继续 T16。
