...
- T46 已开始：验证资料热度进入排行榜、管理员手动重建总榜、下架状态流转及普通用户管理员接口拒绝；用户提供的 PDF 保持为公开演示资料。
- T46 已完成真实排行榜与管理员联调：PDF 资料进入热门资料总榜，`UML` 进入日热门搜索词榜；普通用户调用重建和下架均返回 HTTP 403；管理员总榜重建成功；另一条已审核测试资料下架后退出公开详情、搜索与热榜，并写入审计记录。PDF 保持公开可用；全量前端测试 31 个文件、65 个用例和生产构建均通过，下一步进入 T47 运行说明整理。
- T47 已开始：整理前端安装、环境变量、启动、测试、构建与演示准备说明，并按当前锁定依赖执行最终验证。
- T47 已完成：`frontend/README.md` 已同步当前 Vue 3 + Vite 演示工程的安装、环境变量、启动、测试、构建和演示准备方式；`npm ci` 安装成功且审计 0 漏洞，31 个测试文件、65 个用例和生产构建通过，临时 Vite 首页返回 HTTP 200 后已关闭。前端 T01-T47 自动队列全部完成。
...
已补强：`RankingMapperIntegrationTest` 真实执行 Mapper XML；`RankingTaskExecutionMonitor` 记录当前实例内三个调度入口的最近执行快照和统一日志。

待实现功能：跨实例指标聚合、历史持久化、管理员查询接口、失败告警和遗留 `syncing` 批次监控。
