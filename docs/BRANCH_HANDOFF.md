...
- `frontend/` 已完成 T01-T05；Vite `/api` 代理、Axios JSON 请求层与通用 API 类型已具备，但后端 8080 端口未监听，真实代理联调待补；当前不得提前执行 T07 或后续任务。
...
2. 找到第一个 `TODO`：当前为 `T06`。
3. 仅执行 T06；完成测试、更新计划/队列/状态文档、提交并推送后，才继续 T07。
