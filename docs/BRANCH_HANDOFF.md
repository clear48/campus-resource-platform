...
- `frontend/` 已完成 T01-T37：发布资料页使用后端固定 APPROVED 的公开搜索结果，支持筛选、分页和详情入口且未实现下架按钮。页面测试 1/1 与构建通过；后端 8080 端口未监听，真实联调待补；当前可按队列继续 T38。
...
2. 读取队列中当前 `IN_PROGRESS` 的 T38；完成后将其标记为 DONE 并开始编号最小的 TODO（T39）。
3. 当前仅执行 T38；完成测试、更新计划/队列/状态文档、提交并推送后，才继续 T39。
