...
- `frontend/` 已完成 T01-T30；我的收藏页面已支持详情入口、分页与取消收藏后刷新，页面测试 1/1 与生产构建通过；搜索建议后端尚未实现，前端未导出或调用；后端 8080 端口未监听，真实联调待补；当前可按队列继续 T31。
...
2. 读取队列中当前 `IN_PROGRESS` 的 T31；完成后将其标记为 DONE 并开始编号最小的 TODO（T32）。
3. 当前仅执行 T31；完成测试、更新计划/队列/状态文档、提交并推送后，才继续 T32。
