...
- `frontend/` 已完成 T01-T06；请求层专项测试 3/3 通过，页面组件测试环境已具备；后端 8080 端口未监听，真实代理联调待补；当前不得提前执行 T08 或后续任务。
...
2. 找到第一个 `TODO`：当前为 `T07`。
3. 仅执行 T07；完成测试、更新计划/队列/状态文档、提交并推送后，才继续 T08。
