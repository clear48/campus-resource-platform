...
- 缓存中的 `resourceId`、公开状态和用户态字段会被校验；坏 JSON 或污染值会删除后回源。审核通过、拒绝、下架提交后通过 `invalidate()` 持锁 DELETE 缓存 Key（与回填互斥，锁超时降级直接删除），详情缓存失效先于排行榜派生更新。
...
- `invalidate()` 与 `getOrLoad()` 使用同一把 Redisson 每资料锁保证有序；锁超时（2秒）或 Redisson 不可用时降级为直接 DELETE，Redis 删除失败仅告警，依赖 TTL（30~35分钟）兜底。
...
| 接口名称 | 手动重建热门资料总榜 |
| 请求方法 | `POST` |
| URL | `/api/v1/admin/rankings/resources/hot/rebuild` |
| 是否需要登录 | 是 |
| 权限要求 | 管理员（JWT 中 `role = 2`） |
| 请求参数 | 无 |
...
```json
{
  ...
  "message": "success",
  "data": null,
  "traceId": "rank-rebuild-001"
}
```

说明：接口按 MySQL 中已审核通过资料的下载量、收藏量和浏览量重新构建 Redis `all` 总榜。手动调用会等待已有重建完成后再执行本次重建，不会以“跳过”伪装成功。重建过程持有总榜维护**写锁**并以 `RENAME` 原子替换正式榜；下载、收藏、取消收藏和下架对 `all` 榜的实时写入持有同一读写锁的**读锁**，若重建正在进行会等待替换完成后再写入新榜，避免实时热度被覆盖丢失。日榜、周榜、月榜不受该锁阻塞。
