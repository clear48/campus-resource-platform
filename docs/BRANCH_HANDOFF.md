...
- `frontend/` 已完成 T01-T13；排行榜 API 测试 2/2 与前述基础、认证测试均通过；后端 8080 端口未监听，真实联调待补；当前不得提前执行 T15 或后续任务。
...
2. 找到第一个 `TODO`：当前为 `T14`。
3. 仅执行 T14；完成测试、更新计划/队列/状态文档、提交并推送后，才继续 T15。
