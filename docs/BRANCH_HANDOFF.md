...
| 当前前端任务 | `T05`：创建通用 API 类型和 Axios 请求实例 |
...
- `frontend/` 已完成 T01-T04；Vite `/api` 代理和环境变量已配置，但后端 8080 端口未监听，真实代理联调待补；当前不得提前执行 T06 或后续任务。
...
2. 找到第一个 `TODO`：当前为 `T05`。
3. 仅执行 T05；完成测试、更新计划/队列/状态文档、提交并推送后，才继续 T06。
