...
当前状态：`DONE`（2026-09-24）。

完成情况：

- 候选构建阶段完成后端 208/208、前端 76/76、生产构建与双镜像构建；
- Compose 隔离演练覆盖空卷启动、真实依赖 readiness、最小权限、内部端口隔离、MySQL/Redis/上传目录故障恢复，以及三个持久卷的 `down/up` 保留；
- 旧库迁移按固定顺序执行三份迁移并完整重跑，七张 legacy 表的全列摘要在迁移前、首次迁移后和重跑后完全一致，七类错误结构均被拒绝；
- 联合恢复演练在停止入口和后端后创建同一写入切点，分别备份 MySQL、Redis 完整 AOF 数据卷和上传卷，再恢复到全新空卷；
- 三联 marker 证明切点前数据全部存在、切点后数据全部不存在；从固定提交 `64498230` 构建的上一版镜像通过 revision、健康接口及上传卷读写兼容验证；
- 所有演练均使用唯一 Docker project、随机强 Secret、脱敏 JSON 证据和精确 label 清理，本机 MySQL、Redis 服务不需要启动。

实现提交：`1a8a4e36`、`e2aa4fa7`、`79fb3b2f`、`1e475b18`、`42b39a18`，均已推送到 `origin/deploy`。正式干净演练证据对应迁移提交 `1e475b18` 和回滚提交 `42b39a18`；证据只保存在本机系统临时目录，不提交数据库导出、卷归档或 Secret。

该阶段证明本地合成环境的流程可复现。真实服务器的备份保留、加密、异地保存、RPO/RTO 与生产数据恢复仍属于 `DEPLOY-08`。
