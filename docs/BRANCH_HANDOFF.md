...
- `frontend/` 已完成 T01-T41：排行榜运维页以表格展示两类榜单，周期范围正确；总榜重建有确认但无虚构进度。页面测试 1/1 与构建通过；后端 8080 端口未监听，真实联调待补；当前可按队列继续 T42。
...
2. 读取队列中当前 `IN_PROGRESS` 的 T42；完成后将其标记为 DONE 并开始编号最小的 TODO（T43）。
3. 当前仅执行 T42；完成测试、更新计划/队列/状态文档、提交并推送后，才继续 T43。
