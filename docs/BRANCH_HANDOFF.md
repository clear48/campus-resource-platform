...
- `frontend/` 已完成 T01-T42：路由守卫已覆盖游客、普通用户和管理员入口；前端仅做入口控制，后端仍为最终鉴权边界。路由测试 3/3 与构建通过；后端 8080 端口未监听，真实联调待补；当前可按队列继续 T43。
...
2. 读取队列中当前 `IN_PROGRESS` 的 T43；完成后将其标记为 DONE 并开始编号最小的 TODO（T44）。
3. 当前仅执行 T43；完成测试、更新计划/队列/状态文档、提交并推送后，才继续 T44。
