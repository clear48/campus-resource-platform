...
- `frontend/` 已完成第一阶段 T01-T07；请求层测试 3/3、枚举格式化测试 4/4 通过；后端 8080 端口未监听，真实代理联调待补；当前不得提前执行 T09 或后续任务。
...
2. 找到第一个 `TODO`：当前为 `T08`。
3. 仅执行 T08；完成测试、更新计划/队列/状态文档、提交并推送后，才继续 T09。
