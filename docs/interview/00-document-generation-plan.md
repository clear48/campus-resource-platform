...
| Service 接口 / 实现 | 20 / 20 | 包含排行榜后台任务相关服务 |
| MyBatis Mapper | 9 | 与 9 份 Mapper XML 对应 |
| MySQL 表 | 9 | 以 `sql/init.sql` 为准 |
| Java 测试类 | 27 | 单元、Controller、Mapper/Service 集成测试 |
| 最近现有 Surefire 报告 | 124/124 通过 | 本轮未重新执行测试，只读取已有报告 |
...
| 为什么用 Redisson，不直接用 SETNX | 下载增量同步锁、排行榜维护读写锁、看门狗续期 | 技术栈、问答 |
