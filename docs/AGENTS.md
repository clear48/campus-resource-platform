...
项目级 Agent 配置和完整操作方法见 `docs/08-multi-agent-collaboration.md`。

### 1. 触发条件

满足以下任一条件时，主 Agent 应考虑使用 Subagent：

* 用户明确要求多 Agent 协作；
* 涉及一个完整业务模块或 3 个以上强关联文件；
* 涉及数据库、Redis、事务、并发、权限或安全边界；
* 需要同时进行代码探索、测试分析和只读审查。

简单单文件修改、纯格式调整或无法独立拆分的任务不强制委派，避免无意义增加上下文和令牌消耗。

### 2. 标准流程

1. 分析阶段：`architect` 分析真实调用链；涉及事务、并发或安全时，可同时让 `reviewer` 做只读风险扫描。
2. 方案阶段：主 Agent 等待分析结果并汇总为唯一实施方案，明确文件范围、文件所有权、停止条件和验收标准。
3. 实现阶段：只允许一个 `implementer` 写生产代码，强关联的 Controller、Service、Mapper、SQL 和文档归同一写入者。
4. 验证阶段：实现完成后，`tester` 可补充测试，同时 `reviewer` 只读审查最终差异。
5. 集成阶段：主 Agent 处理最终修复与冲突，运行相关完整验证，并按项目规则提交和推送。

### 3. 文件所有权与工作区

* 同一时刻不得让两个 Agent 修改同一个文件。
* 多个写入 Agent 只有在目录、文件和业务依赖完全不重叠时才能并行。
* `architect` 和 `reviewer` 默认只读；`tester` 原则上只能修改测试目录。
* Subagent 共享当前工作区和未提交修改，不会自动创建独立分支。
* 两个独立功能确需并行开发时，使用两个 Git Worktree；不要把共享工作区的 Subagent 当成 Worktree。
* 开始和提交前都要检查 `git status`，显式暂存本任务文件，不得混入用户或其他 Agent 的改动。

### 4. 协作输出要求

每个 Subagent 返回给主 Agent 的结果必须包含：
...
Subagent 不得只返回“已完成”。主 Agent 的最终总结仍须满足第 20 节，并说明各阶段验证范围。

### 5. 边界与停止条件

* 多 Agent 委派不得扩大用户需求，不得绕过新增依赖、数据库结构、跨模块、安全、权限和事务边界的确认要求。
* 前端任务队列自动推进时，根 `AGENTS.md` 中的适用范围和停止条件优先；Subagent 不能把前端任务扩展到后端、SQL 或 Redis。
* 任何 Agent 发现工作区冲突、业务规则不明确、测试失败或文档与代码冲突时，都必须向主 Agent 返回证据并停止扩大修改。
* Agent 文件中的 `sandbox_mode` 是默认配置；父会话实时选择的权限模式可能覆盖子 Agent 默认值，因此主 Agent 仍须通过任务指令和文件所有权控制写入范围。
