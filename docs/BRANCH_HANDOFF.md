...
- `frontend/` 已完成 T01-T38：下架 API 已通过管理员 Token、路径与请求体 Mock 验证；下架原因页面校验将在 T39 实现；后端 8080 端口未监听，真实联调待补；当前可按队列继续 T39。
...
2. 读取队列中当前 `IN_PROGRESS` 的 T39；完成后将其标记为 DONE 并开始编号最小的 TODO（T40）。
3. 当前仅执行 T39；完成测试、更新计划/队列/状态文档、提交并推送后，才继续 T40。
