...
- `frontend/` 已完成 T01-T43：固定导航按登录态与管理员角色显示入口，管理员端可返回前台，排行榜失败时可重新加载；专项测试 6/6、全量测试 65/65 与构建通过。T44 已验证本机 `127.0.0.1:8080` 未监听且健康检查无法连接，真实联调按停止条件暂停；前端仅做入口控制，后端仍为最终鉴权边界。
...
2. 启动后端、MySQL 和 Redis，并确认 `http://127.0.0.1:8080/api/v1/health` 可访问。
3. 将 T44 从 `BLOCKED` 恢复为 `IN_PROGRESS`，仅执行认证、上传和审核真实联调；不得以 Mock 结果代替人工验收。
